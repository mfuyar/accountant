alter table public.cost_versions
  ...
  add column if not exists payment_fee_amount numeric(14,2);

alter table public.cost_versions
  drop constraint if exists cost_versions_invoice_amount_check,
  drop constraint if exists cost_versions_payment_fee_amount_check;
alter table public.cost_versions
  add constraint cost_versions_invoice_amount_check check (invoice_amount is null or invoice_amount >= 0),
  add constraint cost_versions_payment_fee_amount_check check (payment_fee_amount is null or payment_fee_amount >= 0);

create or replace function public.create_cost_version_v7(
  p_project_id bigint,
  p_cost_id uuid,
  ...
  p_payment_fee_amount numeric default null,
...
begin
  ...
  if p_payment_fee_amount is not null and p_payment_fee_amount < 0 then
    raise exception 'Payment fee amount cannot be negative';
  end if;
  if p_invoice_amount is not null and p_invoice_amount < 0 then
    raise exception 'Invoice amount cannot be negative';
  end if;
  if p_invoice_amount is not null and p_payment_fee_amount is not null
     and abs((p_invoice_amount + p_payment_fee_amount) - p_amount) > 0.009 then
    raise exception 'Invoice amount plus payment fee must equal the total cost';
  end if;
  ...
  if jsonb_array_length(coalesce(p_lot_allocations, '[]'::jsonb)) > 0
    ...
    raise exception 'Lot allocations must equal the total cost amount';
  end if;
  if exists (
  ...
  insert into public.cost_versions (
    ...
    construction_draft_id, payment_method, payment_fee_percentage,
    payment_fee_amount, payment_date, invoice_amount, amount, phase, category,
    ...
    nullif(trim(p_payment_method), ''), p_payment_fee_percentage,
    p_payment_fee_amount, p_payment_date, p_invoice_amount, p_amount, p_phase,
    nullif(trim(p_category), ''), coalesce(p_lot_allocations, '[]'::jsonb),
    p_cost_date, coalesce(p_attachments, '[]'::jsonb),
...
grant execute on function public.create_cost_version_v7(bigint, uuid, uuid, bigint, text, numeric, text, text, jsonb, date, jsonb, boolean, text, uuid, text, numeric, numeric, date, numeric)
  to authenticated;
revoke all on function public.create_cost_version_v7(bigint, uuid, uuid, bigint, text, numeric, text, text, jsonb, date, jsonb, boolean, text, uuid, text, numeric, numeric, date, numeric)
  from public, anon;
